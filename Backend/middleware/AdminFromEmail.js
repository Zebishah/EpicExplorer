...
const getAdminByEmail = async (req, res, next) => {
    ...
    try {
        ...
        if (!admin) {
            return res.status(400).json({ success: false, message: 'No admin found', statusCode: 400 });
        }
        req.admin = admin; // Attach the user object to the request object
        next();
        return;
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Admin not found', statusCode: 400 });
    }
};
